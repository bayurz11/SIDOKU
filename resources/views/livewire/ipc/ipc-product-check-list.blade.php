@php
    ...
    // Ringkasan sederhana dari data di halaman ini
    $collection = $data->getCollection();

    $groupCounts = $collection->groupBy('line_group')->map(fn($items) => $items->count());

    $maxCount = $groupCounts->max() ?: 1;
...
<div class="space-y-6">
    ...
    <div class="bg-white shadow-xl rounded-2xl border border-gray-200 overflow-hidden">
        <div
            class="px-6 py-5 bg-gradient-to-r from-emerald-50 via-blue-50 to-indigo-50 border-b border-gray-200 flex items-center justify-between gap-4">
            <div class="flex items-center gap-4">
                <div
                    class="w-12 h-12 bg-gradient-to-br from-emerald-500 to-blue-600 rounded-xl flex items-center justify-center shadow-lg">
            ...
            <div class="flex items-center gap-4 text-sm">
                <div class="text-right">
                    <div class="text-[11px] uppercase tracking-wide text-gray-500 font-semibold">
                        Total Record (Halaman Ini)
                    </div>
        ...
        <div class="px-6 py-5">
            @if ($groupCounts->isEmpty())
                <p class="text-sm text-gray-500 italic">
                    Belum ada data untuk ditampilkan di chart. Tambahkan atau ubah filter terlebih dahulu.
                </p>
            @else
                <div class="space-y-3">
                    @foreach ($groupCounts as $groupKey => $count)
                        @php
                            $label = $lineGroupLabels[$groupKey] ?? $groupKey;
                            $percent = round(($count / $maxCount) * 100, 1);
                        @endphp
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-1">
                                ...
                                <span class="text-xs text-gray-500">
                                    {{ $count }} record &middot; {{ $percent }}%
                                </span>
                            </div>
                            <div class="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
                                <div class="h-2.5 rounded-full bg-gradient-to-r
                                           @switch($groupKey)
        ...
        <div class="bg-gradient-to-r from-blue-50 via-green-50 to-lime-50 px-6 py-6 border-b border-gray-200">
            ...
            <div class="mt-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1">Sub Line (Teh) @if ($filterLineGroup !== 'LINE_TEH')
                            ...
                            <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center gap-2">
                                    @permission('ipc_product_checks.view')
                                        <button wire:click="showDetail({{ $ipc->id }})"
