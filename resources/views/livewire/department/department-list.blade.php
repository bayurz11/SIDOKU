        ...
        <div class="flex justify-between items-center">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600 mt-1">Kelola data dan status departemen</p>
                </div>
            </div>

            @permission('departments.create')
                <button wire:click="$dispatch('openDepartmentForm')"
        ...
        <div
            ...
            <div class="flex-1">
                <div class="relative">
                    ...
                    <input wire:model.live="search" type="text" placeholder="Cari department…"
                ...
                @forelse($data as $department)
                    <tr
                        ...
                        <td class="px-6 py-5 whitespace-nowrap text-sm text-gray-700">
                            {{ Str::words($department->description ?? '-', 6, '...') }}
                        </td>
                        <td class="px-6 py-5 whitespace-nowrap">
                            <span
                                class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-semibold shadow-sm
                                {{ $department->is_active ? 'bg-gradient-to-r from-green-100 to-emerald-100 text-green-800' : 'bg-gradient-to-r from-red-100 to-pink-100 text-red-800' }}">
                                <div
                                    class="w-2 h-2 rounded-full mr-2 {{ $department->is_active ? 'bg-green-500 animate-pulse' : 'bg-red-500' }}">
                                </div>
                                {{ $department->is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </td>
                        <td class="px-6 py-5 whitespace-nowrap text-sm text-gray-600">
                            <div class="font-medium">{{ $department->created_at->format('d M Y') }}</div>
                            <div class="text-xs">{{ $department->created_at->diffForHumans() }}</div>
                        </td>

                        <td class="px-6 py-5 whitespace-nowrap text-sm font-medium">
                            <div class="flex space-x-2">
                                @permission('departments.edit')
                                    <button wire:click="$dispatch('openDepartmentForm', { id: {{ $department->id }} })"
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center justify-center">
                                ...
                                <h3 class="text-xl font-semibold text-gray-900 mb-2">Belum ada department</h3>
                                <p class="text-gray-500 mb-6 max-w-sm text-center">
                                    @if ($search || !$showInactive)
                                        Coba ubah kata kunci atau filter pencarian.
                                    @else
                                        Tambahkan data department pertama.
                                    @endif
                                </p>
    ...
    <div class="px-6 py-4 bg-gradient-to-r from-gray-50 to-blue-50 border-t border-gray-200 rounded-b-2xl">
        <div class="flex items-center justify-between">
            <div class="text-sm text-gray-600">
                ...
                dari <span class="font-medium">{{ $data->total() }}</span> department
            </div>
